...
% Date: June 2012
%
function GDown_stack = build_GDown_stack_stack4_nofile(Stack, level)
...
frame=squeeze(Stack(1,:,:,:));
    ...
    GDown_stack(1,:,:,:) =  imresize(squeeze(blurred),0.25);
    ...
    for iterator=startIndex+1:endIndex
            ...
            blurred = blurDnClr(frame,level);
            GDown_stack(k,:,:,:) = imresize(squeeze(blurred), 0.25);
